...
def test_ordonare_film():
    ...
    added = Film("1008", "Margelatu", 1980, 15.50, True)
    film_repository.create(added)
    added = Film("1009", "Pistruiatul", 1973, 19.50, True)
    film_repository.create(added)
    added = Film("1010", "Nea Marin miliardar", 1979, 20.00, True)
    film_repository.create(added)
    ...
    rezervare_repository.create(added)
    filme = rezervare_service.ordered_movies_by_number_of_bookings()
    assert len(filme) == 3
    assert filme[0].titlu == "Pistruiatul"
    assert filme[1].titlu == "Nea Marin miliardar"
    assert filme[2].titlu == "Margelatu"
