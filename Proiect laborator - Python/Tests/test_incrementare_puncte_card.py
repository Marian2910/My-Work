...
def test_incrementare_puncte_card_client():
    ...
    card_client_repository.create(added)
    card_client_service.incrementare_puncte_card(datetime.strptime("11.12.1950", '%d.%m.%Y').date(),
                                                 datetime.strptime("11.12.1995", '%d.%m.%Y').date(), 10)
    ...
    card_client_service.incrementare_puncte_card(datetime.strptime("11.12.1999", '%d.%m.%Y').date(),
                                                 datetime.strptime("11.12.2010", '%d.%m.%Y').date(), 20)
    assert len(card_client_repository.read()) == 3
    assert card_client_repository.read()[0].puncte_acumulate == 22
    assert card_client_repository.read()[1].puncte_acumulate == 10
