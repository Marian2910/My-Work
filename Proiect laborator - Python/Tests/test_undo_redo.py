...
def test_undo_redo():
    ...
    film_repository = RepositoryJson(film_file)
    card_client_repository = RepositoryJson(card_file)
    rezervare_repository = RepositoryJson(rezervre_file)
    rezervare_validator = RezervareValidator(film_repository, card_client_repository)
    ...
    card_client_service = CardClientService(card_client_repository, card_client_validator, undo_redo_service)
    rezervare_service = RezervareService(rezervare_repository, rezervare_validator,
                                         film_repository, card_client_repository, undo_redo_service)
    film_service.add_film("1005", "B.D. in alerta", 1971, 24.50, True)
    film_service.add_film("1006", "Baltagul", 1969, 29.50, True)
    film_service.add_film("1009", "Nea Marin miliardar", 1979, 20.00, True)
    ...
    card_client_service.update_card("1006", "Mutu", "Avraam", 5130601060889,
                                    datetime.strptime("01.06.2013", '%d.%m.%Y').date(),
                                    datetime.strptime("12.12.2021", '%d.%m.%Y').date(), 0)

    assert card_client_repository.read("1006").nume == "Mutu"
    assert card_client_repository.read("1006").prenume == "Avraam"
    assert card_client_repository.read("1006").puncte_acumulate == 0
    undo_redo_service.do_undo()
    assert card_client_repository.read("1006").nume == "Buga"
    ...
    rezervare_service.stergere_rezervari_interval_zile(datetime.strptime("01.10.1900", '%d.%m.%Y').date(),
                                                       datetime.strptime("30.11.2022", '%d.%m.%Y').date())
