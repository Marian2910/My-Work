...
def test_stergere_rezervare_interval_data():
    ...
    film_repository = RepositoryJson(filename1)
    card_client_repository = RepositoryJson(filename2)
    rezervare_repository = RepositoryJson(filename3)
    ...
    added = Film("1005", "B.D. in alerta", 1971, 24.50, True)
    film_repository.create(added)
    added = Film("1006", "Baltagul", 1969, 29.50, True)
    film_repository.create(added)
    added = Film("1009", "Nea Marin miliardar", 1979, 20.00, False)
    film_repository.create(added)
    ...
    rezervare_service.stergere_rezervari_interval_zile(datetime.strptime("01.10.2021", '%d.%m.%Y').date(),
                                                       datetime.strptime("30.11.2021", '%d.%m.%Y').date())
    assert len(rezervare_repository.read()) == 3
    rezervare_service.stergere_rezervari_interval_zile(datetime.strptime("01.12.2021", '%d.%m.%Y').date(),
                                                       datetime.strptime("02.12.2021", '%d.%m.%Y').date())
    assert len(rezervare_repository.read()) == 3
    rezervare_service.stergere_rezervari_interval_zile(datetime.strptime("01.12.2019", '%d.%m.%Y').date(),
                                                       datetime.strptime("14.12.2022", '%d.%m.%Y').date())
