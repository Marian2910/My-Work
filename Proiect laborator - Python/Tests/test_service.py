...
def test_card_client_service_add():
    ...
    try:
        film_service.add_film("", "Mistery Murder", 2019, 100, False)
        assert False
    except Exception as e:
        assert True
        assert str(e) == "['ID-ul nu poate fi vid!']"
    try:
        film_service.add_film("1002", "Mistery Murder", -2019, -100, False)
        assert False
    except Exception as ex:
        assert True
        assert str(ex) == "['Pretul biletului trebuie sa fie un numar pozitiv!'," \
                          " 'Anul de aparatie nu poate sa fie un numar negativ!']"
