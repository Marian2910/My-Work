...
class CardClientService:
    ...
    def update_card(self, id_client: str, nume_new: str, prenume_new: str, cnp_new: int,
                    data_nasterii_new: date, data_inregistrarii_new: date, puncte_acumulate_new: int) -> None:
        """
        ...
        self.undo_redo_service.clear_redo()
        updated_operation = UpdateOperation(self.card_client_repository, card, card_before)
    ...
    def delete_card(self, id_client: str) -> None:
        ...
        delete_operation = DeleteOperation(self.card_client_repository,
                                           self.card_client_repository.read(id_client))
    ...
    def get_ordered_descending_by_card_points(self) -> List[OrderedCards]:
        """
        ordoneaza cardurile client dupa punctele acumulate
        :return:
        """
        carduri = self.get_all_card()
        ordonare_carduri = map(lambda puncte: OrderedCards(puncte.nume,
                                                           puncte.prenume, puncte.puncte_acumulate), carduri)
        return sorted(ordonare_carduri, key=lambda x: x.points, reverse=True)

    def incrementare_puncte_card(self, data_start: date, data_stop: date, val: int) -> None:
        """
        :param data_start:
        :param data_stop:
        :param val:
        :return:
        """
        carduri_clienti = self.get_all_card()
        ...
        self.undo_redo_service.clear_redo()
        updated_operation = UpdateOperationLoop(self.card_client_repository,
                                                carduri_clienti, self.card_client_repository.read())
        self.undo_redo_service.add_to_undo(updated_operation)
